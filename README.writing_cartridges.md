...
All cartridges expose ports to provide service to related gears in an application. 
The exposed ports are available via environment variables corresponding to the name 
specified for the mapping. All mappings route within a node to an assigned internal 
IP address.

Mappings are namespaced. The full mapping identifier is generated by OpenShift and 
follows the form `OPENSHIFT_{CART_NS}_{PORT_NAME}:{PORT}`. The `{CART_NS}` segment 
is generated according to [TODO: DOC EXISTING ALGORITHM]. The `{PORT_NAME}` segment 
is specified by the cartridge metadata. The port assignment itself is random.

Mappings are defined via the cartridge `manifest.yml` file, in the `Exposed-Ports` 
section. Each entry is a key-value pair, where the key is the name of the port, and 
the value is the port to map.
...
```
OPENSHIFT_CUSTOMCART_PROXY_PORT=<assigned external IP>:<assigned port 0>
